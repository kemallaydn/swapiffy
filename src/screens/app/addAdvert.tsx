...
function Favorites() {
  ...
  const [advert, setAdvert] = useState({ title: "", category: "", imageurl: null, description: "", status: "", userid: null });
  ...
  async function onDisplayNotification() {
    await axios.post('http://localhost:8080/v1/advertisements/add', advert).then((response) => {
  ...
  useEffect(() => {
    if (authState.isLoggedIn) {
      ...
      handleChnage({ target: { name: "userid", value: authState.data.authenticatedUser.id } });
    }
    if (imageData != null) {
