...
function Favorites() {
  ...
  const [advert, setAdvert] = useState({ title: "", category: "", imageurl: null, description: "", status: "", userid: id, location: null });
  ...
  useEffect(() => {
    const getLocation = async () => {
      const locations = await useLocation();
      setAdvert({ ...advert, location: locations.latitude + "," + locations.longitude });
    };
    getLocation();
  }, [])
  useEffect(() => {
  ...
  return (
    <Container >
      <CustomView>
        ...
        <TextInput placeholder="Ürün Adı" onChangeText={(e: any) => {
          handleChnage({ target: { name: "title", value: e } });
